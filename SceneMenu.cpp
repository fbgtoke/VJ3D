...
void SceneMenu::initScene() {
  ...
  mOptions[0].setPositionInTiles(glm::vec3(0, 0, 0));
  ...
  mOptions[1].setPositionInTiles(glm::vec3(0, -1, 0));
  ...
  mOptions[2].setPositionInTiles(glm::vec3(0, -2, 0));
  ...
  mOptions[3].setPositionInTiles(glm::vec3(0, -3, 0));
}
...
void SceneMenu::updateScene(int deltaTime) {
  ...
  for (int i = 0; i < NUM_OPTIONS; ++i) {
    if (i == mCurrentOption) {
      mOptions[i].setScale(1.2f);
      mOptions[i].setRotationSpeed(glm::vec3(0.005f, 0.f, 0.f));
    } else {
      mOptions[i].setScale(1.f);
